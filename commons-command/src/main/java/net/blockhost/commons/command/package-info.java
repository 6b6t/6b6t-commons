...
/// <pre>
/// `public class HelpSubCommand implements SubCommand{void execute(@NotNull Player player, @NotNull String[] args){player.sendMessage("Available commands: ...");}@NotNull String getName(){return "help";}@NotNull String[] getAliases(){return new String[]{"?", "h"};}}`</pre>
///
/// Create and register the dispatcher:
/// <pre>
/// `CommandDispatcher dispatcher = CommandDispatcher.builder().defaultSubCommand("help").unknownSubCommandHandler((player, args) ->player.sendMessage("Unknown command: " + args[0])).register(new HelpSubCommand()).register(new TeleportSubCommand()).build();plugin.getCommand("mycommand").setExecutor(dispatcher);plugin.getCommand("mycommand").setTabCompleter(dispatcher);`</pre>
///
