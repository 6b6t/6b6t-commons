...
/// <pre>
/// `class PluginConfig{settings")private DatabaseConfig database = new DatabaseConfig();public DatabaseConfig database(){return database;}}// Load config and create connection poolPluginConfig config = ConfigLoader.loadOrCreate(configPath, PluginConfig.class);HikariDataSource dataSource = HikariDataSourceBuilder.createDataSource(config.database());`</pre>
/// ## Manual Configuration
///
/// For simple, non-pooled connections:
/// <pre>
/// `DatabaseCredentials credentials = DatabaseCredentials.builder().host("localhost").port(3306).database("mydb").username("user").password("pass").build();try (Connection conn = MariaDbConnectionFactory.openConnection(credentials)){// Use the connection}`</pre>
///
/// For connection pooling with HikariCP:
/// <pre>
/// `HikariDataSource dataSource = HikariDataSourceBuilder.create(credentials).poolName("MyPlugin-Pool").maximumPoolSize(10).build();try (Connection conn = dataSource.getConnection()){// Use the connection}// Remember to close the data source when donedataSource.close();`</pre>
///
